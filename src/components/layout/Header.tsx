...
export default function Header({ profile, onMenuClick }: { profile: Profile | null; onMenuClick?: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-20 bg-surface/80 backdrop-blur-md border-b border-border px-4 md:px-6 h-14 md:h-16 flex items-center gap-3 md:gap-4">
      ...
      {/* Search */}
      <div className="hidden md:flex items-center gap-2 bg-s2 border border-border rounded-xl px-3 py-2 w-56 hover:border-accent/40 transition-colors group">
        ...
        <span className="text-tx-3 text-xs flex-1">Search leads...</span>
        <kbd className="text-tx-3 text-xs bg-s3 border border-border px-1.5 py-0.5 rounded-md font-mono">⌘K</kbd>
      </div>

      {/* Notification bell */}
      <button className="relative p-2 rounded-xl border border-border bg-s2 text-tx-3 hover:text-tx hover:border-accent/40 transition-all">
